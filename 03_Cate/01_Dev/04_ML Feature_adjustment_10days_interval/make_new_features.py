...
def get_df_with_features(
    ...
    w_price_vol_corr_long = weighted_price_pivot.rolling(LONG_PERIOD).corr(vol_pivot)
    w_price_vol_corr_mid = weighted_price_pivot.rolling(MID_PERIOD).corr(vol_pivot)
    w_price_vol_corr_short = weighted_price_pivot.rolling(SHORT_PERIOD).corr(vol_pivot)
    ...
    close_sma_short = close_pivot.rolling(SHORT_PERIOD).mean()
    close_sma_mid = close_pivot.rolling(MID_PERIOD).mean()
    close_sma_long = close_pivot.rolling(LONG_PERIOD).mean()

    close_sma_mid_over_long = close_sma_mid / close_sma_long
    clsoe_sma_short_over_mid = close_sma_short / close_sma_mid

    close_sma_mid_larger_long_count_short = close_sma_mid_over_long.map(lambda x : 1 if x > 1 else 0).rolling(SHORT_PERIOD).sum()
    close_sma_mid_larger_long_count_mid = close_sma_mid_over_long.map(lambda x : 1 if x > 1 else 0).rolling(MID_PERIOD).sum()
    close_sma_mid_larger_long_count_long = close_sma_mid_over_long.map(lambda x : 1 if x > 1 else 0).rolling(LONG_PERIOD).sum()

    close_sma_short_larger_mid_count_short = clsoe_sma_short_over_mid.map(lambda x : 1 if x > 1 else 0).rolling(SHORT_PERIOD).sum()
    close_sma_short_larger_mid_count_mid = clsoe_sma_short_over_mid.map(lambda x : 1 if x > 1 else 0).rolling(MID_PERIOD).sum()
    close_sma_short_larger_mid_count_long = clsoe_sma_short_over_mid.map(lambda x : 1 if x > 1 else 0).rolling(LONG_PERIOD).sum()
    ...
    ##
    close_mean_short_pivot = change_close_pivot.rolling(SHORT_PERIOD).mean()
    close_mean_mid_pivot = change_close_pivot.rolling(MID_PERIOD).mean()
    close_mean_long_pivot = change_close_pivot.rolling(LONG_PERIOD).mean()

    close_min_ratio_short = close_pivot.rolling(SHORT_PERIOD).min() / close_pivot
    close_min_ratio_mid = close_pivot.rolling(MID_PERIOD).min() / close_pivot
    close_min_ratio_long = close_pivot.rolling(LONG_PERIOD).min() / close_pivot

    close_max_ratio_short = close_pivot.rolling(SHORT_PERIOD).max() / close_pivot
    close_max_ratio_mid = close_pivot.rolling(MID_PERIOD).max() / close_pivot
    close_max_ratio_long = close_pivot.rolling(LONG_PERIOD).max() / close_pivot
    ...
    kospi_sma_5 = kospi_pivot.rolling(5).mean()
    kospi_sma_10 = kospi_pivot.rolling(10).mean()

    kosdaq_sma_5 = kosdaq_pivot.rolling(5).mean()
    kosdaq_sma_10 = kosdaq_pivot.rolling(10).mean()

    Kosdaq_sma_10_pct_change_3 = kosdaq_sma_10.pct_change(3)
    Kosdaq_sma_10_pct_change_1 = kosdaq_sma_10.pct_change(1)
    ...
    # Max in 10days
    close_shift = close_pivot.shift(-10)
    close_max_in_10days = close_pivot.shift(-10).rolling(10).max()
    max_rtn_in_10days = close_max_in_10days / close_pivot - 1

    close_max_in_5days = close_pivot.shift(-5).rolling(5).max()
    max_rtn_in_5days = close_max_in_5days / close_pivot - 1
    ...
    def get_future_rtn_high(p):
        return(
            high_pivot.shift(-p) / close_pivot - 1
        )
    ...
    new_features_concatenated = pd.concat(
        [
            ...
            get_stacked_df(close_mean_short_pivot, 'close_mean_short'),
            get_stacked_df(close_mean_mid_pivot, 'close_mean_mid'),
            get_stacked_df(close_mean_long_pivot, 'close_mean_long'),

            get_stacked_df(close_min_ratio_short, 'close_min_ratio_short'),
            get_stacked_df(close_min_ratio_mid, 'close_min_ratio_mid'),
            get_stacked_df(close_min_ratio_long, 'close_min_ratio_long'),

            get_stacked_df(close_max_ratio_short, 'close_max_ratio_short'),
            get_stacked_df(close_max_ratio_mid, 'close_max_ratio_mid'),
            get_stacked_df(close_max_ratio_long, 'close_max_ratio_long'),
            ...
            get_stacked_df(close_sma_mid_over_long, 'close_sma_mid_over_long'),
            get_stacked_df(clsoe_sma_short_over_mid, 'clsoe_sma_short_over_mid'),

            get_stacked_df(close_sma_mid_larger_long_count_short, 'close_sma_mid_larger_long_count_short'),
            get_stacked_df(close_sma_mid_larger_long_count_mid, 'close_sma_mid_larger_long_count_mid'),
            get_stacked_df(close_sma_mid_larger_long_count_long, 'close_sma_mid_larger_long_count_long'),

            get_stacked_df(close_sma_short_larger_mid_count_short, 'close_sma_short_larger_mid_count_short'),
            get_stacked_df(close_sma_short_larger_mid_count_mid, 'close_sma_short_larger_mid_count_mid'),
            get_stacked_df(close_sma_short_larger_mid_count_long, 'close_sma_short_larger_mid_count_long'),

            get_stacked_df(kosdaq_sma_5, 'kosdaq_sma_5'),
            get_stacked_df(kosdaq_sma_10, 'kosdaq_sma_10'),

            get_stacked_df(Kosdaq_sma_10_pct_change_1, 'Kosdaq_sma_10_pct_change_1'),
            get_stacked_df(Kosdaq_sma_10_pct_change_3, 'Kosdaq_sma_10_pct_change_3'),

            get_stacked_df(weighted_price_change_p_short, 'weighted_price_change_p_short'),
            get_stacked_df(weighted_price_change_p_mid, 'weighted_price_change_p_mid'),
            get_stacked_df(weighted_price_change_p_long, 'weighted_price_change_p_long'),

            # get_stacked_df(close_shift, 'close_shift'),
            # get_stacked_df(close_max_in_10days, 'close_max_in_10days'),
            get_stacked_df(max_rtn_in_10days, 'max_rtn_in_10days'),
            get_stacked_df(max_rtn_in_5days, 'max_rtn_in_5days'),
            ...
            get_stacked_df(
                get_future_rtn_high(1), 'rtn_high_1'
            ),

            get_stacked_df(
                get_future_rtn_high(2), 'rtn_high_2'
            ),

            get_stacked_df(
                get_future_rtn_high(3), 'rtn_high_3'
            ),
            ...
            # ),
            # get_stacked_df(
            #     get_future_rtn_krx(5), 'rtn_5_krx'
            # ),
            # get_stacked_df(
